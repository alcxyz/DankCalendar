...
PluginComponent {
    ...
    Process {
        ...
        onExited: (exitCode, exitStatus) => {
            if (exitCode === 0) {
                ...
                try {
                    var json = JSON.parse(root._syncOutput.trim());
                    root.hasError = true;
                    root.errorText = json.error || "Credential sync failed";
                } catch (e) {
                    root.hasError = true;
                    root.errorText = "Credential sync failed";
                }
            }
        }
    }
    ...
    function fetchEvents() {
        _pendingOutput = "";
        fetchProc.command = ["dankcalendar", "list", "--days", "" + root.lookAheadDays];
    ...
    Process {
        ...
        onExited: (exitCode, exitStatus) => {
            try {
                ...
                root.hasError = true;
                root.errorText = root._pendingOutput.trim()
                    ? "Parse error"
                    : "Configure your CalDAV account in plugin settings";
            }
    ...
    function fetchCalendars() {
        _calOutput = "";
        calProc.command = ["dankcalendar", "calendars"];
    ...
    function checkNotifications() {
        _notifyOutput = "";
        notifyProc.command = ["dankcalendar", "notify", "--before", "" + root.notifyMinutes];
    ...
    // Returns "YYYY-MM-DDTHH:MM" — used by add for timed events
    function formatISO(d) {
        return d.getFullYear() + "-" + pad2(d.getMonth() + 1) + "-" + pad2(d.getDate()) +
               "T" + pad2(d.getHours()) + ":" + pad2(d.getMinutes());
    }

    // Returns "YYYY-MM-DD" — used by add for all-day events
    function formatISODate(d) {
        return d.getFullYear() + "-" + pad2(d.getMonth() + 1) + "-" + pad2(d.getDate());
    }

    // Returns "YYYYMMDD" — used by edit --start-date / --end-date
    function formatQcalDate(d) {
        return "" + d.getFullYear() + pad2(d.getMonth() + 1) + pad2(d.getDate());
    }

    // Returns "HHMM" — used by edit --start-time / --end-time
    ...
    function addEvent() {
        ...
        _addOutput = "";
        var cmd = ["dankcalendar", "add",
            "--calendar", "" + addCalendarIdx,
            "--summary", addTitle];

        if (addAllDay) {
            cmd = cmd.concat(["--start", formatISODate(addStartDate), "--all-day"]);
        } else {
            cmd = cmd.concat(["--start", formatISO(addStartDate), "--end", formatISO(addEndDate)]);
        }
    ...
    function saveEdit() {
        ...
        var cmdArgs = ["dankcalendar", "edit",
            "--calendar", "" + editEvent.calendarIndex,
    ...
    function deleteEvent() {
        if (!editEvent || !editEvent.filename) return;
        _deleteOutput = "";
        deleteProc.command = ["dankcalendar", "delete",
            "--calendar", "" + editEvent.calendarIndex,
            "--filename", editEvent.filename];
